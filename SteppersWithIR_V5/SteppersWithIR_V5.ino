  ...
  // Method which takes the IR codes and turns them into commands
    ...
    switch (irCode) {
      ...
      case MOVE_HOME:
        ...
        runMotors = true;
        oled.clear();
        oled.println("Move Home and Stop.");
        break;
      case MOVE_STOP:
...
void CoordinateSwitchToX() {
  ...
  oled.clear();
  oled.println("Enter Coordinates");
  oled.println("X:_");
  oled.println("");
  oled.println("SETUP when Done");
}//CoordinateSwitchToX
...
void CoordinateSwitchToDone() {
  ...
  digitalWrite (LEDpin, LOW);
  coordinateStatus = USER_DONE;
  oled.println("Press ENTER to Move.");
}//CoordinateSwitchToDone
...
void DisplayMainMenu() {
  oled.clear();
  oled.println("o_o  AutoScope  o_o");
  oled.println("Use Arrows and Stop");
  oled.println("Setup for direct entry");
  oled.println("Back to Move Home");  
}
